...
namespace IBS.DataAccess.Repository.Filpride
{
    public class BankAccountRepository: Repository<BankAccount>, IBankAccountRepository
    {
        ...
        public async Task<List<SelectListItem>> GetBankAccountListAsync(string company,
            CancellationToken cancellationToken = default)
        {
            return await _db.BankAccounts
                .Where(a => company == nameof(Filpride))
                .Select(ba => new SelectListItem { Value = ba.BankAccountId.ToString(), Text = ba.AccountName })
