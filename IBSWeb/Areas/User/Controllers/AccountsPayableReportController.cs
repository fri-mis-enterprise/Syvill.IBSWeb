    ...
    public class AccountsPayableReportController: Controller
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                        ...
                        page.Content().PaddingTop(10).Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Category").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Subcategory").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Payee").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Date").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Voucher#").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Bank Name").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Check").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Particulars").SemiBold();
                                header.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignCenter()
                                    .AlignMiddle().Text("Total").SemiBold();
                            });
                            ...
                            table.Cell().Background(Colors.Grey.Lighten1).Border(0.5f).Padding(3).AlignRight()
                                .Text(totalAmt != 0
                                    ? totalAmt < 0
                                        ? $"({Math.Abs(totalAmt).ToString(SD.Two_Decimal_Format)})"
                                        : totalAmt.ToString(SD.Two_Decimal_Format)
                                    : null).SemiBold().FontColor(totalAmt < 0 ? Colors.Red.Medium : Colors.Black);
                ...
                AuditTrail auditTrailBook = new(GetUserFullName(), "Generate cleared disbursement report quest pdf",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex,
                    "Failed to generate cleared disbursement report quest pdf. Error: {ErrorMessage}, Stack: {StackTrace}. Generated by: {UserName}",
        ...
        public async Task<IActionResult> GenerateClearedDisbursementReportExcelFile(ViewModelBook model,
            CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                AuditTrail auditTrailBook = new(GetUserFullName(), "Generate cleared disbursement report excel file",
                    "Accounts Payable Report");
                await _unitOfWork.AuditTrail.AddAsync(auditTrailBook, cancellationToken);
                ...
                var fileName =
                    $"Cleared_Disbursement_Report_{DateTimeHelper.GetCurrentPhilippineTime():yyyyddMMHHmmss}.xlsx";
            ...
            catch (Exception ex)
            {
                TempData["error"] = ex.Message;
                _logger.LogError(ex,
                    "Failed to generate cleared disbursement report excel file. Error: {ErrorMessage}, Stack: {StackTrace}. Generated by: {UserName}",
                    ex.Message, ex.StackTrace, _userManager.GetUserName(User));
                return RedirectToAction(nameof(ClearedDisbursementReport));
            }
        }
        ...
        public async Task<IActionResult> GenerateNonTradeInvoiceReportExcelFile(ViewModelBook model,
            CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                var payments = await _dbContext.CheckVoucherHeaders
                    ...
                    .Select(x => new { x.Reference, x.CheckVoucherHeaderNo, x.DcrDate })
                    .ToListAsync(cancellationToken);
                ...
                col++;
                worksheet.Cells[row, col].Value = "INV NO.";
                col++;
                worksheet.Cells[row, col].Value = "CV PAYMENT";
                col++;
                worksheet.Cells[row, col].Value = "DCR";
                col++;
                worksheet.Cells[row, col].Value = "PAYEE";
                col++;
                worksheet.Cells[row, col].Value = "PARTICULARS";
                col++;
                worksheet.Cells[row, col].Value = "DOCUMENT TYPE";
                col++;
                worksheet.Cells[row, col].Value = "ACCOUNT NUMBER";
                col++;
                worksheet.Cells[row, col].Value = "ACCOUNT NAME";
                col++;
                worksheet.Cells[row, col].Value = "SUB ACCOUNT NAME";
                col++;
                worksheet.Cells[row, col].Value = "DEBIT";
                col++;
                worksheet.Cells[row, col].Value = "CREDIT";
                col++;
                ...
                {
                    worksheet.Cells[row, col].Value = "VOIDED BY";
                    col++;
                ...
                foreach (var inv in nonTradeInvoiceReport)
                {
                    ...
                    worksheet.Cells[row, col].Style.Numberformat.Format = "MMM/dd/yyyy";
                    col++;
                    worksheet.Cells[row, col].Value = inv.CheckVoucherHeader.CheckVoucherHeaderNo;
                    col++;
                    worksheet.Cells[row, col].Value = paymentInfo?.CheckVoucherHeaderNo ?? "";
                    col++;
                    ...
                    col++;
                    worksheet.Cells[row, col].Value = inv.CheckVoucherHeader.Type;
                    col++;
                    worksheet.Cells[row, col].Value = inv.AccountNo;
                    col++;
                    worksheet.Cells[row, col].Value = inv.AccountName;
                    col++;
                    worksheet.Cells[row, col].Value = inv.SubAccountName;
                    col++;
                    ...
                    if (showVoidCancelColumns)
                    {
                        worksheet.Cells[row, col].Value = inv.CheckVoucherHeader.VoidedBy;
                        col++;
                ...
                AuditTrail auditTrailBook = new(GetUserFullName(), "Generate Non-Trade Invoice report excel file",
                    "Accounts Payable Report");
                await _unitOfWork.AuditTrail.AddAsync(auditTrailBook, cancellationToken);
                ...
                var fileName =
                    $"NonTrade_Invoice_Report_{DateTimeHelper.GetCurrentPhilippineTime():yyyyddMMHHmmss}.xlsx";
                var stream = new MemoryStream();
                await package.SaveAsAsync(stream, cancellationToken);
                stream.Position = 0;
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", fileName);
            ...
            catch (Exception ex)
            {
                TempData["error"] = ex.Message;
                _logger.LogError(ex,
                    "Failed to generate non trade invoice report excel file. Error: {ErrorMessage}, Stack: {StackTrace}. Generated by: {UserName}",
                    ex.Message, ex.StackTrace, _userManager.GetUserName(User));
                return RedirectToAction(nameof(NonTradeInvoiceReport));
            }
        }
        ...
        public async Task<IActionResult> GenerateCvDisbursementReportExcelFile(ViewModelBook model,
            CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                worksheet.Cells[row, col].Value = "DATE";
                col++;
                worksheet.Cells[row, col].Value = "CV No.";
                col++;
                worksheet.Cells[row, col].Value = "DCR DATE";
                col++;
                worksheet.Cells[row, col].Value = "CHECK #";
                col++;
                worksheet.Cells[row, col].Value = "PAYEE";
                col++;
                worksheet.Cells[row, col].Value = "PARTICULARS";
                ...
                worksheet.Cells[row, col].Value = "DOCUMENT TYPE";
                col++;
                worksheet.Cells[row, col].Value = "INVOICE No.";
                col++;
                worksheet.Cells[row, col].Value = "ACCOUNT NUMBER";
                col++;
                worksheet.Cells[row, col].Value = "ACCOUNT NAME";
                ...
                foreach (var header in cvTradeHeaderReport)
                {
                    ...
                    {
                        col = 1;
                        worksheet.Cells[row, col].Value = header.Date.ToDateTime(TimeOnly.MinValue);
                        col++;
                        worksheet.Cells[row, col].Value = header.CheckVoucherHeaderNo;
                        col++;
                        worksheet.Cells[row, col].Value = header.DcrDate?.ToDateTime(TimeOnly.MinValue);
                        col++;
                        worksheet.Cells[row, col].Value = header.CheckNo;
                        col++;
                        worksheet.Cells[row, col].Value = header.Payee;
                        col++;
                        worksheet.Cells[row, col].Value = header.Particulars;
                        worksheet.Cells[row, col].Style.WrapText = true;
                        col++;
                        worksheet.Cells[row, col].Value =
                            header.Type == nameof(DocumentType.Documented) ? "Doc" : "Undoc";
                        col++;
                        worksheet.Cells[row, col].Value = header.Reference;
                        worksheet.Cells[row, col].Style.WrapText = true;
                        col++;

                        worksheet.Cells[row, col].Value = details.AccountNo;
                        col++;
                        worksheet.Cells[row, col].Value = details.AccountName;
                        col++;
                        worksheet.Cells[row, col].Value = details.SubAccountName;
                        col++;
                        worksheet.Cells[row, col].Value = details.Debit;
                        col++;
                        worksheet.Cells[row, col].Value = details.Credit;
                        col++;
                        ...
                        if (showVoidCancelColumns)
                        {
                            worksheet.Cells[row, col].Value = header.VoidedBy;
                            col++;
                ...
                AuditTrail auditTrailBook = new(GetUserFullName(), "Generate Cv Disbursement report excel file",
                    "Accounts Payable Report");
                await _unitOfWork.AuditTrail.AddAsync(auditTrailBook, cancellationToken);
                ...
                var fileName =
                    $"CV_Disbursement_Report_{DateTimeHelper.GetCurrentPhilippineTime():yyyyddMMHHmmss}.xlsx";
                var stream = new MemoryStream();
                await package.SaveAsAsync(stream, cancellationToken);
                stream.Position = 0;
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", fileName);
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateJournalVoucherExcelFile(ViewModelBook model,
            CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                foreach (var detail in journalVoucherReport)
                {
                    ...
                    worksheet.Cells[row, 11].Value =
                        detail.JournalVoucherHeader.CheckVoucherHeader?.CheckVoucherHeaderNo;
            ...
            catch (Exception ex)
            {
                TempData["error"] = ex.Message;
                _logger.LogError(ex,
                    "Failed to generate journal voucher report excel file. Error: {ErrorMessage}, Stack: {StackTrace}. Generated by: {UserName}",
                    ex.Message, ex.StackTrace, _userManager.GetUserName(User));
                return RedirectToAction(nameof(JournalVoucherReport));
            }
        }
